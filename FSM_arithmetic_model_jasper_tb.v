...
// Version     : 3.0
//   – X-propagation check fixed: checks result/error_flag outputs, not input reg
//   – X-injection wrapped in a proper req transaction so DUT is in PROCESS
//   – All magic 2'b?? literals replaced with named localparams
//   – Intermediate regs used for all loop-variable task arguments (xmvlog safe)
//   – SVA assertion failure counter included in final report
// =============================================================================
...
module tb_jaspergold_complete;
    ...
    // Verilog does not allow part-selects or arithmetic expressions as task
    // arguments (causes xmvlog EXPRPA / EXPSMC / MISEXX errors).
    // =========================================================================
    ...
    // These fire every posedge clk during simulation and count failures.
    // =========================================================================

    // P1: IDLE -> PROCESS on req
    SVA_IDLE_TO_PROCESS: assert property (
        @(posedge clk) disable iff (rst)
        (dut.current_state == ST_IDLE && req) |=> (dut.current_state == ST_PROCESS)
    ) else begin
    ...
    SVA_PROCESS_TO_DONE: assert property (
        @(posedge clk) disable iff (rst)
        (dut.current_state == ST_PROCESS && power_enable && secure_access)
        |=> (dut.current_state == ST_DONE)
    ) else begin
    ...
    SVA_PROCESS_TO_ERROR_POWER: assert property (
        @(posedge clk) disable iff (rst)
        (dut.current_state == ST_PROCESS && !power_enable)
        |=> (dut.current_state == ST_ERROR)
    ) else begin
    ...
    SVA_PROCESS_TO_ERROR_SEC: assert property (
        @(posedge clk) disable iff (rst)
        (dut.current_state == ST_PROCESS && !secure_access)
        |=> (dut.current_state == ST_ERROR)
    ) else begin
    ...
    SVA_DONE_TO_IDLE: assert property (
        @(posedge clk) disable iff (rst)
        (dut.current_state == ST_DONE) |=> (dut.current_state == ST_IDLE)
    ) else begin
    ...
    SVA_ERROR_TO_IDLE: assert property (
        @(posedge clk) disable iff (rst)
        (dut.current_state == ST_ERROR) |=> (dut.current_state == ST_IDLE)
    ) else begin
    ...
    SVA_GRANT_IN_PROCESS: assert property (
        @(posedge clk) disable iff (rst)
        (dut.current_state == ST_PROCESS) |-> grant
    ) else begin
    ...
    SVA_VALID_IN_DONE: assert property (
        @(posedge clk) disable iff (rst)
        (dut.current_state == ST_DONE) |-> valid
    ) else begin
    ...
    SVA_ERROR_ON_INSECURE: assert property (
        @(posedge clk) disable iff (rst)
        (dut.current_state == ST_PROCESS && !secure_access) |-> error_flag
    ) else begin
    ...
    SVA_ERROR_IN_ERROR_STATE: assert property (
        @(posedge clk) disable iff (rst)
        (dut.current_state == ST_ERROR) |-> error_flag
    ) else begin
    ...
    SVA_RESET_TO_IDLE: assert property (
        @(posedge clk)
        $rose(rst) |=> (dut.current_state == ST_IDLE)
    ) else begin
    ...
    //   Injects X into data_a DURING a live PROCESS transaction so the DUT
    //   actually computes with the unknown value.  Checks whether X propagated
    //   to result or error_flag outputs – NOT just whether data_a is X.
    //   (This fixes the original superficial check: "if (^data_a === 1'bx)")
    // =========================================================================
    task inject_x_and_check;
        input [DATA_W-1:0] b;
        input [1:0]        op;
        begin
            // Start a transaction with X in data_a
            data_a        = {DATA_W{1'bx}};
            data_b        = b;
            opcode        = op;
            secure_access = 1'b1;
            ...
            @(posedge clk); #1;   // FSM in PROCESS; DUT computes with X input
            ...
            if ((^result === 1'bx) || (^error_flag === 1'bx)) begin
                ...
                $display("[X-PROP] X contained – result=%0d error_flag=%b",
                         result, error_flag);
            end
    ...
    initial begin
        ...
        for (i = 0; i < 4; i = i + 1) begin
            for (j = 0; j < 16; j = j + 1) begin
                ...
                // Normal operation (secure, powered)
                ...
                // -------------------------------------------------------
                // X-State injection – FIXED:
                //   inject_x_and_check drives data_a=X INTO a real PROCESS
                //   transaction and checks whether X propagated to DUT
                //   outputs (result, error_flag), NOT just the input reg.
                // -------------------------------------------------------
                inject_x_and_check(arg_b, arg_op);
            end
        end
