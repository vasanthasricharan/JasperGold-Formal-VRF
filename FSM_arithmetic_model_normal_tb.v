...
// Version     : 2.0 (improved – full corner coverage, parameterized, signed SUB)
// =============================================================================

// Local defines to avoid magic numbers
...
module tb_normal_simulation;
    ...
    // =========================================================================
    // TASK: run_one_op
    //   Applies one full IDLE->PROCESS->DONE transaction and checks result.
    // =========================================================================
    task run_one_op;
        input [DATA_W-1:0] a;
        input [DATA_W-1:0] b;
        ...
        begin
            ...
            req = 1'b0;
            @(posedge clk); #1;   // let FSM advance to DONE / ERROR
    ...
    initial begin
        ...
        // Using non-trivial values; include a subtract that would underflow
        // unsigned (3 - 5) to verify signed handling.
        run_one_op(4'd3, 4'd1, 2'b00, 1'b1, 1'b1);  // ADD   3+1  = 4
        run_one_op(4'd3, 4'd5, 2'b01, 1'b1, 1'b1);  // SUB   3-5  = -2 (signed)
        ...
        // Min values: 0 OP 0
        run_one_op(4'd0, 4'd0,  2'b00, 1'b1, 1'b1);   // ADD  0+0
        ...
        data_a = {DATA_W{1'b0}};  // restore

        // --- (F) Deadlock detection ----------------------------------------
        // After a complete transaction the FSM must return to IDLE.
        // Drive another request; if valid is seen the FSM is alive.
        ...
        $display("# Verification Scope  : Functional + Security + Power + Boundary + X-State                                  #");
